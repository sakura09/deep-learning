...
# reduction='none'告诉函数不要对损失值进行任何降维操作。损失函数会为每个样本返回一个损失值，结果是一个与样本数量相同长度的张量
# 默认情况下，如果不指定参数或者设置为'mean'会计算所有样本损失的平均值，'sum'会计算所有样本损失的总和
...
def _get_batch_loss_bert(net, loss, vocab_size, tokens_X,
    ...
    _, mlm_Y_hat, nsp_Y_hat = net(tokens_X, segments_X,
                                  ...
                                  pred_positions_X)
    # 计算遮蔽语言模型损失 跟weight乘一下，被pad的那些就不去算loss
...
def train_bert(train_iter, net, loss, vocab_size, devices, num_steps):
    ...
    # 这里没用epoch可能是因为正常来说Bert数据太大了，所以用num_steps就是算了多少个batch
